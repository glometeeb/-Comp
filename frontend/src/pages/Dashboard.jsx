...
function fmt(n) {
  if (n == null) return '—';
  return n < 0 ? `(${Math.abs(n).toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 })})` : n.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 });
}
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {jobs?.map(job => (
          <Link key={job.id} to={`/jobs/${job.id}`} className="bg-white rounded-xl shadow hover:shadow-md border border-gray-100 p-5 transition flex flex-col gap-3">
