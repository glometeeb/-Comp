...
router.post('/', requireAuth, requireEditor, upload.single('file'), async (req, res) => {
  ...
  if (!req.body.job_number?.trim()) return res.status(400).json({ error: 'Job number is required' });
  ...
  try {
    ...
    previews.set(token, {
      parsed,
      job_name: req.body.job_name || req.file.originalname.replace(/\.xlsx?$/, ''),
    ...
    res.json({
      token,
      phases: parsed.phases.map(p => ({ phaseName: p.phaseName, phaseCode: p.phaseCode, lineCount: p.lines.length })),
...
router.post('/confirm', requireAuth, requireEditor, async (req, res) => {
  ...
  if (!preview) return res.status(400).json({ error: 'Invalid or expired upload token. Please re-upload the file.' });

  const name = job_name || preview.job_name;
  const num = (job_number || preview.job_number || '').trim();
  if (!num) return res.status(400).json({ error: 'Job number is required' });
  ...
  for (const phase of preview.parsed.phases) {
    const { data: phaseRow, error: phaseErr } = await supabase
      ...
      .insert({
        ...
        is_single_phase: phase.isSinglePhase,
    ...
    if (phase.lines.length > 0) {
      await supabase.from('cost_code_lines').insert(phase.lines.map(l => ({ phase_id: phaseRow.id, ...l })));
    }
  }
